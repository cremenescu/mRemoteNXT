...
RDPCore *rdpcore_create(const char *host, int port, const char *user,
    ...
    // TCP keepalive. FreeRDP's defaults are 5s idle then 3 probes 2s apart, and on macOS
    // they really are applied per socket (tcp.c defines TCP_KEEPIDLE as TCP_KEEPALIVE on
    // Apple, so the system's 2h idle default is overridden). Eleven seconds of silence is
    // far too little for an interactive session — a tab parked on a Windows pre-logon
    // screen paints nothing at all, and aggressive probes are also the ones stateful
    // firewalls are most likely to drop, since they deliberately carry an out-of-window
    // sequence number. 60s idle + 4 probes 15s apart still catches a dead path in ~2min.
